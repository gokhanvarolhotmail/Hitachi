...
INNER JOIN( SELECT
              ...
              , ( SELECT SUBSTRING([definition_Replaced], NULLIF([b].[NWL], 0), DATALENGTH([definition_Replaced]) - [b].[NWL] - [b].[NWR] + 2) AS [OutVal]
                  FROM( SELECT
                          ...
                          , PATINDEX('%[^' + CHAR(32) + CHAR(9) + CHAR(13) + CHAR(10) + ']%', REVERSE([definition_Replaced])) AS [NWR] ) AS [b] ) AS [definition_Replaced_2]
            FROM( SELECT *, REPLACE([m].[definition], '''', '''''') AS [definition_Replaced] FROM [sys].[all_sql_modules] AS [m] ) AS [m] ) AS [m] ON [m].[object_id] = [o].[object_id]
WHERE 1 = 1 ;
GO
-- Definition Scripts
...
SELECT
    CONCAT(
      ...
      , CONVERT(VARCHAR(30), [o].[modify_date], 121), ''', ''', [m].[definition_Replaced_2], '''),') AS [SQL]
FROM [sys].[objects] AS [o]
INNER JOIN [sys].[schemas] AS [s] ON [o].[schema_id] = [s].[schema_id]
INNER JOIN( SELECT
                *
