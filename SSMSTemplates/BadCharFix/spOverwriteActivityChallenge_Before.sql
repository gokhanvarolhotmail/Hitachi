/****** Object:  StoredProcedure [dbo].[spOverwriteActivityChallenge]    Script Date: 7/30/2021 11:26:29 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN TRY
    ...
    BEGIN
		...
		MERGE [dbo].[STG_ActivityChallenge] as [target]
				USING
				(select * from [dbo].[STG_ActivityChallenge_LOAD]) as [source]
				ON (
				...
				WHEN MATCHED AND (
									 [source].[dacadoo_ID] is not null
									 and [source].[EventID] is not null
									 and [source].[ContactID] is not null
								 ) THEN --Null record values do not merge and serve no function in rewarding so they are excluded
					UPDATE SET 
